...
/*
	...
	Related Files:
		...
		u-boot/include/db12x.h (CONFIG_BOOTARGS, )
		u-boot/httpd/cgi.c  (KERNEL_LOCATION)

	Type	Start Address	End Address		Size
	U-Boot	0x00000000		0x00020000		 128	kbytes
	nvram	0x00020000		0x00030000		  64	kbytes
	linux	0x00030000		0x001b0000		1536	kbytes
	rootfs	0x001b0000		0x007b0000		6144	kbytes
	LANG	0x007b0000		0x007e0000		 192	kbytes
	MAC		0x007e0000		0x007f0000		  64	kbytes
	ART		0x007f0000		0x00800000		  64	kbytes
	
#define CONFIG_BOOTARGS     "console=ttyS0,115200 root=31:03 mem=64M rootfstype=squashfs,jffs2 init=/sbin/init mtdparts=ar7240-nor0:192k(u-boot),64k(nvram),1536k(linux),5120k(rootfs),896k(PLC),64k(PIB),192k(LANG),64k(MAC),64k(ART)"
*/
...
#define CONFIG_ROOTFS_TYPE "rootfstype=squashfs,jffs2"
#else
...
#	define MTDPARTS_DEFAULT		"mtdparts=ath-nor0:128k(u-boot),64k(nvram),1536k(linux),6144k(rootfs),192k(LANG),64k(MAC),64k(ART)"
#       define CONFIG_ROOT_BOOTARGS	"root=31:03 mem=32M " CONFIG_ROOTFS_TYPE
#ifdef BUILD_OPTIMIZED_4M
#	define BOOT_IMG_UPPER_BOUND	64 * 1024 //for upgrade bootloader in backup mode
#else
...
#	else // NOT CONFIG_MI124 and NOT CONFIG_AP123
#		ifdef COMPRESSED_UBOOT 
#			define CONFIG_EXTRA_ENV_SETTINGS		\
			"dir=\0"					\
			"lu=tftp 0x80060000 ${dir}tuboot.bin&&"		\
			...
			"lf=tftp 0x80060000 ${dir}db12x${bc}-jffs2&&"	\
				"erase 0x9f1b0000 +0x600000;"		\
				"cp.b $fileaddr 0x9f050000 $filesize\0"	\
			"lk=tftp 0x80060000 ${dir}vmlinux${bc}.lzma.uImage&&"	\
				"erase 0x9f020000 +$filesize;"		\
				"cp.b $fileaddr 0x9f020000 $filesize\0"	\
			""
#		else // not COMPRESSED_UBOOT
#	define CONFIG_EXTRA_ENV_SETTINGS		\
	"dir=\0"					\
	"lu=tftp 0x80060000 ${dir}u-boot.bin&&"		\
		"erase 0x9f000000 +$filesize;"		\
...
// #       define CONFIG_ROOT_BOOTARGS	"root=31:02 " CONFIG_ROOTFS_TYPE
#	define MTDPARTS_DEFAULT		"mtdparts=ath-nor0:128k(u-boot),64k(nvram),1536k(linux),6144k(rootfs),192k(LANG),64k(MAC),64k(ART)"
#       define CONFIG_ROOT_BOOTARGS	"root=31:03 " CONFIG_ROOTFS_TYPE
#	define BOOT_IMG_UPPER_BOUND	128 * 1024 //for upgrade bootloader in backup mode
// ----------------------------------------------------------------------------------------------------------
#	endif /*CONFIG_MI124*/
#else
...
#define CONFIG_BOOTCOMMAND "bootm 0x9f020000"
#endif
#else  /* NOT FLASH_SIZE == 4 and NOT FLASH_SIZE == 8*/
/*
*  Date: 2011-030-21 
*  Name: Charles Teng
*  Reason: patch from LSDK-9.2.0.303
...
/* Backup mode httpd support , this will be adjust by u-boot/Makefile */
...
/* reference : ap121.h , initialize wireless PCI interface will cause loader bootup kernel failed */
#ifdef COMPRESSED_UBOOT
