...
*   Note    : Copy from AR7161
*/
#if defined(HTTPD_SUPPORT) && defined(UIP_PKT_LEN)
...
/*  Date    :
*   Name    : Jimmy Huang
*   Reason  : Let backup mode can receieve multicast
*   Note    : Copy from AR7161
*/
//    ag7240_reg_wr(mac, AG7240_MAC_FIFO_CFG_5, 0x7eccf); // this can not receive multicast
    ag7240_reg_wr(mac, AG7240_MAC_FIFO_CFG_5, 0x3ffff); /* default value , enable receive multicast (for arp) */
...
static int ag7240_check_link(ag7240_mac_t *mac)
{
	...
	DEBUG_MSG("	mac->mac_unit = %d (%x)\n",mac->mac_unit);
	DEBUG_MSG("	mac->duplex = %d (%x)\n",mac->duplex);
	DEBUG_MSG("	mac->speed  = %d (%x)\n\n\n",mac->speed);
	DEBUG_MSG("	duplex = %d (%x)\n",duplex);
	DEBUG_MSG("	speed  = %d (%x)\n\n\n",speed);
	DEBUG_MSG("	Force to set link = upm speed  = _1000BASET , duplex = 1 , \n\n");
...
*/
#ifndef CONFIG_AP123
// this setting is nothing related to backup mode it only related to PCB design
#if defined(HTTPD_SUPPORT)
	// Force to bring eth up
	mac->link = 1;
