...
SUBLEVEL = 4
EXTRAVERSION =
# Add SVN rev number for identify u-boot local version
SVN_VERSION=`svn info 2>/dev/null | grep '^Last Changed Rev' |sed 's/^Last Changed//'`
#U_BOOT_VERSION = $(VERSION).$(PATCHLEVEL).$(SUBLEVEL)$(EXTRAVERSION)
U_BOOT_VERSION = $(VERSION).$(PATCHLEVEL).$(SUBLEVEL)$(EXTRAVERSION)$(SVN_VERSION)
...
# jimmy add for Backup mode support
ifeq ($(HTTPD_SUPPORT), 1)
# loader mode httpd
...
ifeq ($(COMPRESSED_UBOOT),1)
all:	pre_config $(ALL) tuboot.bin
else
all:	pre_config $(ALL)
endif

# jimmy add for Backup mode support
# config u-boot/include/configs/db12x.h, enable / disable HTTPD_SUPPORT
pre_config:
# jery add copy bsp.h to ar7240 for read MAC_WAN_PORT
	-cp -f $(WWW_PATH)/bsp.h	board/ar7240/common/

ifeq ($(HTTPD_SUPPORT), 1)
#	@echo "SUBDIRS = $(SUBDIRS) !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!"
#	@echo "platform = $(PLATFORM)"
#	@echo "HTTPD_SUPPORT = $(HTTPD_SUPPORT)"
#	@echo "LODAER_CONFIG = $(LODAER_CONFIG)"
#	echo "## doing SUBDIRS, PLATFORM = $(PLATFORM)"
#	echo "## doing SUBDIRS, WWW_PATH = $(WWW_PATH)"
	-cp -f $(WWW_PATH)/defaultConf.h ./httpd
	-cp -f $(WWW_PATH)/bsp.h	./httpd
	@if [ -n "`cat $(LODAER_CONFIG) |grep '#define HTTPD_SUPPORT'`" ] ;then \
		echo "do has httpd support !!" ; \
		sed -e "s/^#define HTTPD_SUPPORT.*/#define HTTPD_SUPPORT 1/g" $(LODAER_CONFIG) > $(LODAER_CONFIG).bak || exit 1; \
		mv $(LODAER_CONFIG).bak $(LODAER_CONFIG) ; \
	else \
		echo "" ; \
		echo "$(LODAER_CONFIG) does not has the define \"HTTPD_SUPPORT\", please manually add it !" ; \
		echo "" ; \
	fi
else
	@if [ -n "`cat $(LODAER_CONFIG) |grep '#define HTTPD_SUPPORT'`" ] ;then \
		sed -e "s/^#define HTTPD_SUPPORT.*/#define HTTPD_SUPPORT 0/g" $(LODAER_CONFIG) > $(LODAER_CONFIG).bak || exit 1 ; \
		$(LODAER_CONFIG).bak $(LODAER_CONFIG) \
	fi
endif
...
u-boot:		version depend $(SUBDIRS) $(OBJS) $(LIBS) $(LDSCRIPT)
...
ifneq ($(findstring mi124,$(BUILD_CONFIG_TYPE)),)
	@echo '#define CONFIG_MI124 1' >>include/config.h
endif
ifneq ($(findstring ap123,$(BUILD_CONFIG_TYPE)),)
...
ifdef BUILD_OPTIMIZED_4M
	@echo "#define BUILD_OPTIMIZED_4M " >>include/config.h
endif
...
ifeq ($(CONFIG_HORNET_1_0_WAR), 1)
	@echo "#define CONFIG_HORNET_1_0_WAR 1" >>include/config.h
endif
ifeq ($(CONFIG_HORNET_1_1_WAR), 1)
	@echo "#define CONFIG_HORNET_1_1_WAR 1" >>include/config.h
endif
ifeq ($(CONFIG_HORNET_ETH), 1)
	@echo "#define CONFIG_HORNET_ETH 1" >>include/config.h
endif
#/*
#*  Date: 2011-030-21 
#*  Name: Charles Teng
