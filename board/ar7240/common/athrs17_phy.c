...
void athrs17_reg_init()
{
    ...
    /*  JimmyHuang 20110811
    	Enable LAN Port LED Control 
    */
    athrs17_reg_write(0x10  , 0xc1000000);

    athrs17_reg_write(0x4   , 0x07600000);
    athrs17_reg_write(0xc   , 0x01000000);
    athrs17_reg_write(0x7c  , 0x0000007e);

    /* jery: Disable WAN port in u-boot for block WAN port packets pass to LAN side */
    if (MAC_WAN_PORT==5)
        athrs17_reg_write(0x90  , 0x00000000);
    else if (MAC_WAN_PORT==1)
        athrs17_reg_write(0x80  , 0x00000000);
